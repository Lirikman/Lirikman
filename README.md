<div id="header" align=center>
  <h1>Hi there, I'm Dmitriy</h1>
</div>

<h3>Образование:</h3>
Хакасский технический институт, специализация - экономист

<h3>Дополнительное образование:</h3>
<ul type="square">
  <li>Hexlet - Основы Python</li>
  <li>PythonUniversity - Курс "Python как профессия"</li>
  <li>Изучал программирование на Python по книге Марка Лутса - Изучаем Python</li>
  <li>Изучил разработку нейросетей в академии THE FOUNDER</li>
  <li>Прошёл курс 'Разработка Golang для продвинтых' на платформе Hexlet</li>
</ul>

<h3>Опыт работы:</h3>
  <p> Написание учебных проектов:</p>
<ul>
  <p>- приложение для прасинга HH.RU<p>
  <p>- чат-бот для Telegram</p>
  <p>- сайт на Flask</p>
  <p>- сайт на Django</p>
  <p>- модель нейросети для распознавания букв русского алфавита</p>
  <p>- развертывание сервера Amnezia WireGuard на Ubuntu</p>
  <p>- CLI-утилита "Анализатор размера диска"</p>
  <p>- CLI-утилита "Генератор отличий"</p>
  <p>- web-сервис "Сокращатель ссылок"</p>
  <p>- CLI-утилита "Парсер сайтов"</p>
</ul>

<h3>О себе:</h3>
<p>Работаю региаонльным менеджеров в ПАО "Ак Барс банк"</p>
<p>Предыдущее место работы - АО "РМ Рейл Абаканвагонмаш", ведущий инженер-программист </p>
<p>Имею навыки разработки нейронных сетей, а также програмирование на Python</p>
<p>Постоянно слежу за новостями мира информационных технологий - Habr, 3DNews, iXBT</p>
<p>В настоящее время изучаю разработку на языке Golang</p>

<h3>Ключевые навыки:</h3>
<ul type="disc">
  <li>Знание архитектуры ПК</li>
  <li>Знание операционных систем семейства Windows</li>
  <li>Знание основ Linux</li>
  <li>Знание сетевых технологий</li>
  <li>Знание основ языка запросов SQL</li>
  <li>Знание основ языка программирования Python</li>
  <li>Знание основ языка программирования Golang</li>
</ul>


<!--
**Lirikman/Lirikman** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
